...
main() {
  ...
  then
      ...
      CMD='docker run --rm
          ...
          --name tiler gehlenborglab/ome-tiff-tiler:v0.0.6'
      echo "Running: $CMD"
      eval $CMD
      # vitessce relies on this naming strategy, whereas the docker image is more general
      mv "$TILES_PATH/tiff.json" "$OUTPUT/vanderbilt.raster.json" || \
        sudo mv "$TILES_PATH/tiff.json" "$OUTPUT/vanderbilt.raster.json"
  fi
}
